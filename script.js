...
async function consultarMEF() {
    try {
        const response = await fetch(`data_mef.json?v=${Date.now()}`);
        todosLosProyectos = await response.json();
        const anios = [...new Set(todosLosProyectos.map(p => p.anio))].sort((a,b) => b-a);
        document.getElementById('select-anio').innerHTML = anios.map(a => `<option value="${a}">${a}</option>`).join('');
        filtrarTodo();
...
function actualizarKPIs(lista) {
    const tPim = lista.reduce((a, p) => a + p.pim, 0);
    const tDev = lista.reduce((a, p) => a + p.devengado, 0);
...
function actualizarGraficos(lista) {
    ...
    chartSectores = new Chart(document.getElementById('chartSectores'), {
        ...
        options: { 
            ...
            scales: { x: { ticks: { autoSkip: false, maxRotation: 45, minRotation: 45, font: { size: 10 } } } }
        }
    });
        ...
        <div class="col">
            <div class="proyecto-card">
                ...
                <div class="metricas-box">
                    <div class="d-flex justify-content-between"><span>PIM:</span><span>S/ ${p.pim.toLocaleString('es-PE')}</span></div>
                    <div class="d-flex justify-content-between text-primary"><span>DEV:</span><span>S/ ${p.devengado.toLocaleString('es-PE')}</span></div>
                    <div class="d-flex justify-content-between mt-1"><span class="text-muted">Avance:</span><span style="color:${color}">${p.avance}%</span></div>
